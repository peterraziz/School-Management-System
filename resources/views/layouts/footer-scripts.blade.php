...
<script>
    $(document).ready(function () {
        $('select[name="Grade_id"]').on('change', function () {
            ...
            if (Grade_id) {
                $.ajax({
                    url: "{{ URL::to('Get_classrooms2') }}/" + Grade_id,
                    type: "GET",
                    dataType: "json",
                    success: function (data) {
                        $('select[name="Classroom_id"]').empty();
...
<script>
    $(document).ready(function () {
        $('select[name="Classroom_id"]').on('change', function () {
            ...
            if (Classroom_id) {
                $.ajax({
                    url: "{{ URL::to('Get_Sections2') }}/" + Classroom_id,
                    type: "GET",
                    dataType: "json",
                    success: function (data) {
                        $('select[name="section_id"]').empty();
...
<script>
    var currentLanguage = document.documentElement.lang; // or set this based on your own logic
    
    // Set the position based on the language
...
{{--  TO change the plz fill this fiels, but its not working
<script>
    ...
    form.addEventListener('submit', function (event) {
        ...
        form.querySelectorAll('input[required], textarea[required]').forEach(function (input) {
            if (!input.value.trim()) {
                ...
                // Set custom message based on locale
                var message = '{{ app()->getLocale() == "ar" ? "هذا الحقل مطلوب" : "This field is required" }}';
                input.setCustomValidity(message);
                input.reportValidity();
            } else {
                input.setCustomValidity(''); // Clear custom validity if input has a value
            }
        });
    });
});
